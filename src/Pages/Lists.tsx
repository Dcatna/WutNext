    ...
    return (
       ...
       <Link to={`/home/list/${item.list_id}`} state={item}>
            <div className='w-full rounded-md hover:bg-black/50 text-center flex flex-relative p-1'>   
                ...
                <div className='grid grid-cols-2 w-[65px] rounded-lg overflow-hidden'>
                {posters.map((post) => (
                     <div className='w-full relative'>
                        <img src={`https://image.tmdb.org/t/p/original//${post}`} alt="" className='w-full h-full object-cover aspect-1'/>
                    </div>
                ))} </div>: <div className='w-[65px] grid grid-cols rounded-lg overflow-hidden'><img src={movieicon} className='w-full h-full object-cover aspect-1'></img></div>}   
